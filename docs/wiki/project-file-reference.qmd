...
| `understory.enabled` | boolean | `false` | **Enable**: If checked, the understory module is enabled. |
| `understory.spinupSteps` | numeric | `0` | **Spinup steps**: Set the number of steps (years) for the understory spinup at startup. |
| `understory.pftFile` | file selection | `pft.csv` | **PFT File**: File containing the definition of the understory plant functional types (PFTs). |
| `understory.statesFile` | file selection | `states.csv` | **States File**: File containing the definition of the developmental states of the PFTs. |
...
## Understory (`output.understory`)

Understory cover and state per resource unit and year.

| **XML Key** | **Type** | **Default Value** | **Description** |
| :--- | :--- | :--- | :--- |
| `enabled` | boolean | `false` | **Enabled**: Enable the understory output. |
| `condition` | function | `condition understory` | **Condition**: Controls if the output should be created for the current year (variable: year). |
| `conditionRU` | function | `condition understory RU` | **Condition RU**: can be used to suppress resource-unit-level details; eg. specifying 'in(year,100,200,300)' limits output on reosurce unit level to the years 100,200,300 (leaving 'conditionRU' blank enables details per default). |
