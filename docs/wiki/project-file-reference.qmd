...
::: {.callout-note}
This page is a detailed tabular reference of all XML settings keys. For a general overview of the project file structure and its sections, see the [Project File Structure Guide](project-file.qmd).
:::
...
| `lip` | directory selection | `LIP file directory` | **LIP files**: Directory containing the binary LIP files. See [Lightroom](lightroom.qmd) about creating LIPS! Default: "lip" |
...
Defines the main input database, that primarily stores the species parameter [Species Parameter](species-parameter.qmd)

| **XML Key** | **Type** | **Default Value** | **Description** |
| :--- | :--- | :--- | :--- |
| `in` | file selection | `input database file` | **Input**: Defines the main input database, that primarily stores the species parameter. [Species Parameter](species-parameter.qmd) |
| `out` | file selection | `output database file` | **Output**: Results of simulations are written to this database [Organizing Outputs](organizing-outputs.qmd) |
| `climate` | file selection | `climate database file` | **Climate**: The SQLite database containing the climate data driving the simulation [Climatedata](climatedata.qmd) |
...
This section deals with Javascript-script files that are loaded into the models' script engine at startup. See [Iland Scripting](iland-scripting.qmd)
...
Define the width and height (in m) of the simulated area [Simulation Extent](simulation-extent.qmd)
...
The table name in the climate database and a SQL filter to select a subset of data. See environment file mechanism for spatial data [Simulation Extent](simulation-extent.qmd#spatially_distributed_parameters)
...
| `dayLengthLimit` | numeric | `14.5` | **Day length limit**: Threshold day length (hours) after which autumn generation development stops. Default is 14.5 hours. |
...
Describes how to set up initial tree vegetation in the model. See [Initialize Trees](initialize-trees.qmd)
...
| `seedBackgroundFilter` | string |  | **Seed Background Filter**: An expression (variable 'year') that controls when global background seed input is active (e.g., 'year<=5'). If empty, background seeds are always active if configured. |
...
This section contains parameters for the ICBM/2N carbon and nitrogen cycling sub model, and snags. More Details: Carbon and nitrogen cycling: [Soil C And N Cycling](soil-c-and-n-cycling.qmd) Soil parameterization: [Soil Parametrization And Initialization](soil-parametrization-and-initialization.qmd) Individual tracking of dead trees: [Dead Trees](dead-trees.qmd)
...
This section identifies the source of the species parameters and configures climate response parameters. Note that nitrogenResponseClasses and phenology settings cannot be edited in this GUI settings editor and must be edited directly in the XML project file. For details on these complex settings, see [Project File](project-file.qmd#species)
...
simulates the ignition, spread and the impacts of wildfire on the landscape scale. See [Wildfire](wildfire.qmd) for conceptual and [Fire Module](fire-module.qmd) for technical documentation.
...
The wind modules simulates the disturbance agent wind on landscape level. See [Wind Disturbance](wind-disturbance.qmd) and [Wind Module](wind-module.qmd) for modeling concept and implementation, respectively.
...
The barkbeetle module simulates a specific biotic agent, namely the European Spruce bark beetles (Ips typographus). See [Bark Beetle Disturbance](bark-beetle-disturbance.qmd) and [Barkbeetle Module](barkbeetle-module.qmd) for details.
...
| `winterMortalityShareUnderBark` | numeric | `1.0` | **Share under bark**: Share (0..1) of beetles overwintering under tree bark. Default is 1.0. |
| `winterMortalityShareInSoil` | numeric | `0.0` | **Share in soil**: Share (0..1) of beetles overwintering in soil/litter layer. Default is 0.0. |
| `winterMortalityInSoil` | numeric | `0.05` | **Mortality in soil**: Fixed winter mortality rate (0..1) for beetles overwintering in soil. Default is 0.05. |
| `airTempMaxLimit` | numeric | `16.5` | **Air temp max limit**: Air temperature threshold (°C) required for swarming and brood establishment. Default is 16.5. |
| `tempSumLimit` | numeric | `140.3` | **Degree-day threshold**: Cumulative degree-day sum needed to start spring development. Default is 140.3. |
| `ddLimit` | numeric | `557.0` | **Degree-day total limit**: Degree-days required for complete development of a generation. Default is 557.0. |
...
bite, the Biotic Disturbance Engine is a general module to simulate biotic disturbance agents in ecosystem models. See [Bite](bite.qmd) and [Bite](bite.qmd) for more details.
...
Removed indivdual trees [Outputs](outputs.qmd#Tree_Removed_Output)
...
Aggregated vegetation per species for each resource unit [Outputs](outputs.qmd#Stand_by_species_RU)
...
Died trees in current year on the level of RU x species. [Outputs](outputs.qmd#Dead_trees_by_species_RU)
...
Details about the 3PG production submodule on monthly basis and for each species and resource unit. [Outputs](outputs.qmd#Production_per_month_species_and_resource_unit)
...
Sapling cohorts per resource unit [Outputs](outputs.qmd#Sapling_Output)
...
Individual sapling cohorts [Outputs](outputs.qmd#Sapling_Details_Output)
...
Aggregated vegetation per species on landscape level [Outputs](outputs.qmd#Landscape_aggregates_per_species)
...
## Deadtree (`output.deadtree`)

Detailed single dead tree data (standing snags and downed dead wood) [Outputs](outputs.qmd#Dead_Tree_Output)

| **XML Key** | **Type** | **Default Value** | **Description** |
| :--- | :--- | :--- | :--- |
| `enabled` | boolean | `false` | **Enabled**: Output of individual standing and lying dead trees. Use the filter property to reduce amount of data (filter by dead tree variables such as dbh, species, snag, ruindex). The condition property can be used to control output timing (variable 'year'). The output is triggered at the end of the year and after initialization (year 0). |
| `condition` | function |  | **Condition**: Filter expression using variable 'year' to control output timing (e.g. 'in(year, 10, 20)') |
| `filter` | function |  | **Filter**: Filter expression using dead tree variables (e.g. 'dbh>20 and snag=1') |


## Carbon (`output.carbon`)

carbon pools on landscape and/or per resource unit level [Outputs](outputs.qmd#Carbon_and_nitrogen_pools_above_and_belowground_per_RU_yr)
...
User-defined outputs for aggregated tree variables for each stand or species. [Outputs](outputs.qmd#dynamic_stand_output_by_species_RU)
...
`customagg` are advanced outputs, that allow to group different entitites (saplings, trees, RUs) with different aggregation levels. Please edit details directly in the XML project file. [Dynamic Outputs](dynamic-outputs.qmd)
...
Aggregates of all removed trees due to 'natural' death, harvest, or disturbance per species and reason. [Outputs](outputs.qmd#Aggregates_of_removed_trees_due_to_death_harvest_and_disturbances_per_species)
...
Carbon fluxes per resource unit and year and/or aggregated for the full landscape. [Outputs](outputs.qmd#Carbon_fluxes_per_RU_or_landscape_yr)
...
The output contains all carbon inputs to the soil layer (per RU) [Outputs](outputs.qmd#Carbon_input_to_the_soil_per_RU_yr)
...
Annual water cycle output on resource unit/landscape unit. [Outputs](outputs.qmd#Water_output)
...
This output provides annual statistics for each agent [Bite](bite.qmd#Annual_summary_for_each_Bite_agent.) Note that bite allows additional customized outputs per agent [Bite](../bite/index.qmd)
...
Vegetation state transitions on RU over time [Outputs](outputs.qmd#Forest_state_transitions)
...
List of unique states encountered in the simulation [Outputs](outputs.qmd#Unique_forest_states)
...
SVD indicators Indicator data per resource unit as used by SVD. Please edit in XML project file [Outputs](outputs.qmd#SVD_forest_indicator_data) !
