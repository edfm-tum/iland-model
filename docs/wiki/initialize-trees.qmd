...
| snapshot | The whole landscape is loaded from a *vegetation snapshot* (see also [Object Globals](../apidoc/classes/iland/globals.qmd), and the section [## supported initializataion files
...
A *snapshot* is created by using the Javascript function [saveModelSnapshot(](../apidoc/classes/iland/globals.qmd)). This function creates a SQLite database file (which can be quite large) and an additional resource-unit map as a ESRI raster-file with the same name (and .asc as file extension).
To load a snapshot, one can use either the Javascript function [loadModelSnapshot(](../apidoc/classes/iland/globals.qmd)), or use the mode *snapshot* in the project-file for the tree initialization (see top of this page).
