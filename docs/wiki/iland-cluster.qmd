...
More at: https://iland-model.org
(c) Werner Rammer, Rupert Seidl, 2009-2017
compiled: Intel 64 bit Qt 5.8.0

Usage:
ilandc.exe <xml-project-file> <...other options>
Options:
...
See also [iLand console](iland-console.qmd)
```
 
 Alternatively, you can try avoiding building iLand yourself by using the executable package for Linux. In this case, start from the AppImage ( [downloaded for Linux](../download.html)), and:
 
 * extract the AppImage
 ```bash
 chmod +x iLand-x86_64.AppImage
./iLand-x86_64.AppImage --appimage-extract
```
* rename the standard folder to something meaningful, e.g.
```bash 
mv squashfs-root iland-dist
```
* run directly `ilandc` from your script:
```bash
# Path to the extracted directory
ILAND_DIR="/shared/home/user/software/iland-dist"

# ....

# Run ilandc
${ILAND_DIR}/usr/bin/ilandc /path/to/project_file.xml 100

``` 
Note: if you see errors such as `error while loading shared libraries: libQt6Core.so.6: cannot open shared object file`, you may need to run `patchelf` to fix library paths for the binary.
