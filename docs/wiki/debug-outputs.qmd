...
The dynamic output produces statistics for all trees in the simulation. The "dynamic" output is configured in the [project file](project-file.qmd) section "output" (*output.dynamic*) and switched on/off there. In addition, the dynamic debug output can be switched on/off using the [Globals](../apidoc/classes/globals.qmd) Javascript function *Globals.startOutput("debug_dynamic")* and stopOutput().
