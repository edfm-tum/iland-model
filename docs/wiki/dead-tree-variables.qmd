...
| ruindex | numeric index of the resource unit the tree is located on |
| x | position of the tree, x-direction (m) |
| y | position of the tree, y-direction (m) |
| species | tree species (numeric species index; in expressions species names can be used directly, e.g. `species=piab`) |
| snag | `true` (1) if the tree is a standing dead tree, `false` (0) if downed (DWD). |
...
| dbh | DBH (cm) of the tree at the time of death |
