...
### `windEvent(windspeed, winddirection, max_iteration, simulate, current_iteration)` {#windevent}
...
*   **`windspeed`** (`double`): average wind speed (m/s)
*   **`winddirection`** (`double`): wind direction (0°=N..180°=S..270=W°)
*   **`max_iteration`** (`integer`): maximum number of iterations
*   **`simulate`** (`boolean`, optional): if true, trees are not really affected (default: `false`)
*   **`current_iteration`** (`integer`, optional): if given a value >= 0, then only one iteration is calculated ("interactive mode") (default: `-1`)
...
Saves a wind-specific grid (specified by `gridtype`) in ESRI raster textfile format to `filename`.

**Parameters:**

*   **`gridtype`** (`string`): the type of the grid. For possible values see [GUI visualization and grid description](#gui-visualization-and-grid-description).
*   **`filename`** (`string`): destination path for the saved grid (relative to the project root folder).

**Example:**

```javascript
// save the wind fetch grid in ESRI raster format
...
Returns a wind-specific grid (specified by `gridtype`).
...
```javascript
// retrieve wind-specific grid and apply a threshold
var g = Wind.grid('nKilled');
...
Resets and reloads the wind module. Very helpful for repeated simulations as this allows changing model settings.
...
### `setTopexGrid(grid_file_name)` {#settopexgrid}
...
*   **`grid_file_name`** (`string`): path to the ASCII topo grid file.
