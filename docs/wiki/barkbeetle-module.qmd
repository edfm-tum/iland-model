...
### `runBB(iteration)` {#runbb}

**Returns:** `void`

Executes a full cycle of the bark beetle module if `iteration` is 0 (i.e., calculate generations, fetch vegetation from iLand, and spread/attack/kill). If `iteration` is larger than 0, only spread/attack/killing is simulated. This can be used to simulate multiple bark beetle iterations (years) with a static forest state (note, though, that trees are removed! - see also [simulate](#simulate), [newYear](#newyear)).

**Parameters:**

*   **`iteration`** (`integer`): the iteration counter (0 for full cycle, >0 for spread/attack only).
...
Clears and resets the internal grids (10m cells, 100m cells), i.e., damage and spread data.
...
Increments the internal year counter - this is useful when multiple years of bark beetles should be simulated without iLand running (which would synchronize the simulation year).

---

### `gridToFile(type, filename)` {#gridtofile}

**Returns:** `boolean`

Writes a grid of type `type` to an ESRI ASCII grid (`filename`). The project folder is the default location for `filename`.

**Parameters:**

*   **`type`** (`string`): the type of the grid to write. See [Barkbeetle visualization](#barkbeetle-visualization) for the list of available grids.
*   **`filename`** (`string`): the destination file path.
...
Retrieves a grid of the type `type` as a JavaScript object (see also [gridToFile](#gridtofile)).

**Parameters:**

*   **`type`** (`string`): the type of the grid to retrieve.

---

### `generations(x, y)` {#generations}

**Returns:** `double`

Returns the number of bark beetle generations at position x/y (indices on the 10m grid).

**Parameters:**

*   **`x`** (`integer`): local 10m x-coordinate index.
*   **`y`** (`integer`): local 10m y-coordinate index.

---

### `setInfested(x, y)` {#setinfested}

**Returns:** `boolean`

Initiates damage; sets pixel at (x/y) (grid indices) as infested. Returns `true` if the pixel was a potential host pixel.
...
### `setInfestedFromMap(grid, key, probability, agerange)` {#setinfestedfrommap}

**Returns:** `void`

Sets all pixels on stand `key` on the map `grid` as infested with a probability of `probability`. If infested, an "age" for the infestation is randomly selected between 0 and `agerange`.

**Parameters:**

*   **`grid`** ([`Map`](../apidoc/classes/iland/map.qmd)): the map grid object defining the stands.
*   **`key`** (`integer`): the ID/key of the stand on the map.
*   **`probability`** (`double`): the probability (0..1) of infestation for each pixel.
*   **`agerange`** (`integer`): the range of years (0..`agerange`) to randomly choose the infestation age from.

---

### `setBackgroundInfestationProbability(x)` {#setbackgroundinfestationprobability}

**Returns:** `void`

Sets the background infestation probability to `x` for the whole landscape.

**Parameters:**

*   **`x`** (`double`): the background infestation probability (0..1).
