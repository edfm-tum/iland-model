...
There are two primary ways of editing the project file: one is to use the user 
interface ("Edit Settings") which provides a graphical interface for *most* 
settings. The dialog is structured the same way as the hierarchical XML (see below).
The second approach is to directly edit the XML file with an editor. There are
some tools that help with the syntax, e.g. Notepad++, or VS Code. 

You can add comments / notes to the project file, which allows you commenting on
the reasons for a change, different alternative values, literature sources, etc.
Comments are stored as XML-comments (`<-- comment -->`-blocks) directly *above* 
a specific setting. The user interface allows you to edit comments as well (see
the little pen symbol, which is dark if comments are available).

In addition, the project file includes space for a free-text description, where
you can describe briefly the project itself. The description is stored in the
project file, as a (series of) comments at the very top (below the `<xml version='1.0' encoding='utf-8'>` header line). You can use HTML for formatting.


The [project settings reference](project-file-reference.qmd) provides a complete list of settings.


### Example files


An example file is provided with the [example landscape](download.qmd).
