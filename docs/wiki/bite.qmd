...
BITE is coupled with iLand and its implementation makes heavy use of the Javascript Engine provided by Qt (and also used by iLand). The implementation of agents in BITE is purely done in Javascript (which accesses the BITE engine written in C++). A [BiteAgent](http://iland-model.org/bite/#/BiteAgent)consists of several [BiteItems](http://iland-model.org/bite/#/?id=items); each item implements one aspect of the life cycle of an agent.



### Full [BITE documentation](http://iland-model.org/bite)
