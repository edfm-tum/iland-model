...
Note also, that `customagg` is another highly customizable output ([dynamic outputs](dynamic-outputs.qmd)).
...
See [dynamic outputs](dynamic-outputs.qmd) for details.
...
| GPP | double | actually realized gross primary production, kg C; [GPP](primary-production.qmd) including the effect of decreasing productivity with age; note that a rough estimate of [| NPP | double | net primary production, kg C; calculated as NPP=GPP-Ra; Ra, the autotrophic respiration (kg C/ha) is calculated as a fixed fraction of GPP in iLand (see [here](.qmd#sapling-c-and-n-dynamics)(primary-production.qmd) for details). |
| Rh | double | heterotrophic respiration, kg C; sum of C released to the atmosphere from detrital pools, i.e. [| dist_loss | double | disturbance losses, kg C; C that leaves the ecosystem as a result of disturbances, e.g. fire consumption |
...
| NEP | double | net ecosytem productivity kg C, NEP=NPP - Rh - disturbance losses - management losses. Note that NEP is also equal to the total net changes over all ecosystem C pools, as reported in the carbon output (cf. [Randerson et al. 2002](.qmd#snag-decomposition)(http://www.jstor.org/stable/3061028)) |
