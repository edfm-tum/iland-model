...
More general information concerning storage locations and filename conventions is described in [organizing outputs](organizing-outputs.qmd). See also [Debug Outputs](debug-outputs.qmd) for available high-resolution outputs (usually for debugging). Outputs related to the agent based forest management engine are described at the [ABE outputs](abe-outputs.qmd) page. See also [dynamic outputs](dynamic-outputs.qmd) for a highly customizable output, and the [development stages](development-stages.qmd) output for mapping structural forest stages.
...
| understory_c | double | living understory vegetation (e.g. moss) kg C/ha |
...
| lai_saplings | double | LAI (m2/m2) of saplings, calc. from pixels |
| lai_saplings_rus | double | LAI (m2/m2) of saplings, calc. from RUS average |
| lai_understory | double | LAI (m2/m2) of understory layer |
...
## Understory cover per RU/landscape

**Table Name:** **understory**

Understory cover by resource unit / year and/or by landsacpe/year. On resource unit level, the output provides RU proportions, i.e. proportion of stockable area e.g. covered (0..1).

For landscape level outputs, all variables are aggregated over the landscape and scaled to per hectare stockable area. The area column contains the stockable area (per resource unit / landscape) and can be used to scale to values to the actual value on the ground.
...
| rid | integer | id of ressource unit (-1: no ids set) |
| area_ha | double | total stockable area of the resource unit (ha) |
| pft | string | Name of the plant functional type |
| area | double | total area where the PFT is present (based on 2m cells) (% of stockable area) |
| cover | double | total area covered by PFT (area x state-specific cover) (% of stockable area) |
| areaGain | double | total area where PFT regenerated (based on 2m cells, % of stockable area) |
| areaLoss | double | total area with PFT mortality (based on 2m cells, % of stockable area) |
| LAI | double | leaf area index of PFT (m2/m2) |
| biomass | double | biomass of PFT (kg/ha) |
