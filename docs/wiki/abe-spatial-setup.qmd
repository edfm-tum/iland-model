...
Note: if the *agentDataFile* contains additional columns, then the values are automatically assigned to each stand as stand properties and can thus be accessed in Javascript (see [stand.flag() method](../apidoc/classes/stand.qmd)).

There are two ways of specifying the *Agent* for each polygon:

* you can specify an agent directly with the name of the agent in the 'agent' column of the *agentDataFile*. In this case, the agent has to created manually and registered by calling the Javascript function [fmengine.addAgent()](../apidoc/classes/fmengine.qmd).
