...
-   
    ...
    The number of years the current agent is in an outbreak phase. The value is 0 if outbreaks are not simulated (see [LifeCycle](bitelifecycle.qmd) for details).

-   

    ### `agent` ([agent](biteagent.qmd))
...
-   
    ...
    A list of trees (see [treelist](../apidoc/classes/abe/treelist.qmd)) on the cell (trees \>4m). The `trees` can be queried, filtered or modified. The tree list is populated automatically during execution of the agent (but see `reloadTrees()`).
...
-   
    ...
    A list of saplings (tree cohorts \<4m) (see [saplinglist](../apidoc/classes/abe/saplinglist.qmd)) on the cell. The `saplings` can be queried, filtered or modified. The sapling list is populated automatically during execution of the agent (but see `reloadSaplings()`).
