...
```
Similar functionality is provided by the [expr()](tree.qmd#expr)-method of Tree - the difference is mainly performance: while the
[expr()](tree.qmd#expr)-method constructs for each call an internal "expression" (and has to parse the content of the expression each time),
...
| :--- | :--- | :--- |
| [`expression`](#expression) | `string` | A string representing the current expression. The property is read/write and can be used to modify the expression. |

## Methods Overview

| Method | Return Type | Description |
| :--- | :--- | :--- |
| [`TreeExpr()`](#treeexpr) | `void` | Constructs a `TreeExpr`. Use with the `new` JavaScript keyword. |
| [`value(tree)`](#value) | `void` | Calculates the value of the expression for the given `tree`. If the tree is invalid, a warning is written to the log and -1 is returned. |
...
*   **`tree`** (`Tree`): The [Tree](tree.qmd)-object for which to calculate the expression.
