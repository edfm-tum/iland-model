...
| :--- | :--- | :--- |
| [`count`](#count) | `Integer` *(read-only)* | The number of dead tree elements (snags or DWD pieces) currently loaded in the list. |

## Methods Overview

| Method | Return Type | Description |
| :--- | :--- | :--- |
| [`filter(filter)`](#filter) | `Integer` | Applies a filter to the current list of dead trees. Only elements for which the |
| [`loadFromRU(ru_index, loadWhat)`](#loadfromru) | `Integer` | Loads dead trees from a specific resource unit (RU) into the list. |
| [`loadFromStand(stand_id, loadWhat)`](#loadfromstand) | `Integer` | Loads dead trees from a specific stand into the list. |
| [`mean(expression)`](#mean) | `Number` | Calculates the mean value of a given `expression` for all dead tree elements in the list. |
| [`sum(expression)`](#sum) | `Number` | Calculates the sum of a given `expression` for all dead tree elements in the list. |
...
*   **`filter`** (`String`): A valid filter expression (see iLand expression engine documentation).

**Return Value Description:** The number of dead tree elements remaining in the list after applying the filter.
...
### `loadFromRU(ru_index, loadWhat)` {#loadfromru}
...
*   **`ru_index`** (`Integer`): The index of the resource unit from which to load dead trees.
*   **`loadWhat`** (`enum`): Specifies the type of dead wood to load.
    Possible values are:
...
**Return Value Description:** The number of dead tree elements loaded into the list.
...
### `loadFromStand(stand_id, loadWhat)` {#loadfromstand}
...
*   **`stand_id`** (`Integer`): The ID of the stand from which to load dead trees.
*   **`loadWhat`** (`enum`): Specifies the type of dead wood to load.
    Possible values are:
    - `DeadTreeList.Snags`: Load only standing dead trees.
    - `DeadTreeList.DWD`: Load only downed dead wood.
...
*   **`expression`** (`String`): A valid expression (e.g., a dead tree variable name) for which to calculate the mean.

**Return Value Description:** The mean value of the `expression` for the (filtered) dead tree elements.
...
*   **`expression`** (`String`): A valid expression (e.g., a dead tree variable name or '1' for counting)
    for which to calculate the sum.

**Return Value Description:** The sum of the `expression` for the (filtered) dead tree elements.
